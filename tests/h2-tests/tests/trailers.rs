...
#[tokio::test]
async fn poll_trailers_is_woken_by_consuming_data() {
    h2_support::trace_init!();
    ...
    let mock = mock_io::Builder::new()
        ...
        .read(&[
            0, 0, 1, 1, 4, 0, 0, 0, 1, 0x88, 0, 0, 0x0B, 0, 0, 0, 0, 0, 1, 0x68, 0x65, 0x6C, 0x6C,
            0x6F, 0x20, 0x77, 0x6F, 0x72, 0x6C, 0x64, 0, 0, 9, 1, 5, 0, 0, 0, 1, 0x40, 0x84, 0x42,
            0x46, 0x9B, 0x51, 0x82, 0x3F, 0x5F,
        ])
        .build();
    ...
    let body = RefCell::new(response.into_body());

    let mut trailers = task::spawn(poll_fn(|cx| body.borrow_mut().poll_trailers(cx)));
    assert!(trailers.poll().is_pending());
    ...
    assert!(trailers.is_woken());
    let trailers = trailers.poll().map(|result| result.unwrap().unwrap());
