...
impl Recv {
    ...
    pub fn recv_data(
        ...
        // Make the final DATA visible before publishing the closed receive
        // state, so a racing body task cannot observe "closed and empty".
...
pub(super) fn poll_data(
    ...
    match event {
        Some(Event::Data(payload)) => {
            // `poll_trailers` may be parked behind this data frame.
            stream.notify_recv();
