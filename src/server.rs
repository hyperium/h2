...
impl<B: Buf> SendResponse<B> {
    ...
    pub fn send_informational(&mut self, response: Response<()>) -> Result<(), crate::Error> {
        ...
        tracing::trace!(
            "converting informational response to HEADERS frame for stream: {:?}",
            stream_id
        );
        ...
        match &result {
            Ok(()) => tracing::trace!("Successfully sent informational headers"),
            Err(e) => tracing::trace!("Failed to send informational headers: {:?}", e),
        }
